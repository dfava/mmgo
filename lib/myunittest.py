  ...
  def check(semantics, mmgo_program, configuration, dry_run=False, output=None):
    ...
    for test_name in test_names:
      ...
      try:
        ...
        if configuration == '':
          sys.stderr.write("  The configuration is empty\n")
        sys.stderr.write("  semantics='%s'\n" % semantics)
        sys.stderr.write("  program='%s'\n" % mmgo_program)
        if mmgo_program[-3:len(mmgo_program)] == ".py":
          sys.stderr.write("  You are trying to run a .py file as a .mmgo program!\n")
        sys.exit(1)
        #raise e
